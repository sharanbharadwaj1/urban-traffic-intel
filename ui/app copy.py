...
while True:
    ...
    with events_container:
        ...
        for event in events_container:
            ...
            if event["event_type"] == "vehicle_stopped":
                st.error(f"Stopped Vehicle - ID {event['track_id']}")

            elif event["event_type"] == "vehicle_speeding":
                st.warning(f"Overspeed - ID {event['track_id']}")

            else:
                st.warning("No critical events")
